...
def create_course(name):
    course = Course.objects.create(name=name,
                                   short_description='short_description Test',
...
class CoursesListTest(TestCase):
    ...
    def test_courses_view_with_3_courses(self):
        ...
        create_course(name='.NET')
        response = self.client.get('/')
        self.assertQuerysetEqual(response.context['cour'],
                                 ['<Course: Python>', '<Course: Java>', '<Course: .NET>'],
                                 ordered=False)
...
class CoursesDetailTest(TestCase):
    def test_create_lesson(self):
        ...
        lesson = create_lesson(subject='Lesson 1', course=course)
    ...
    def test_coach_assistant_lesson(self):
        ...
        create_lesson(subject='Lesson 1', course=course)
        create_lesson(subject='Lesson 2', course=course)
        response = self.client.get('/courses/1/')
