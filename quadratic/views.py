...
def quadratic_results(request):
    ...
    if type(a) == int and type(b) == int and type(c) == int:
        ...
        if d > 0:
            x1 = (-b + (d ** (1 / 2.0))) / (2 * a)
            x2 = (-b - (d ** (1 / 2.0))) / (2 * a)
            dict_quadratic["two_x"] = "Квадратное уравнение имеет два действительных корня: x1 = %g, x2 = %g" %(x1, x2)
        elif d == 0:
            x = -b / (2 * a)
            dict_quadratic["one_x"] = "Дискриминант равен нулю, квадратное уравнение имеет один действительный корень: x1 = x2 = %g" %(x)
        elif d < 0:
